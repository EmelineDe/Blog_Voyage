CREATE DATABASE IF NOT EXISTS `travel_blog` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;
USE `travel_blog`;

CREATE TABLE `COMMENTAIRES` (
  `idcommentaire` integer  auto_increment,
  `datepublication`datetime DEFAULT CURRENT_TIMESTAMP,
  `comment_contenu` TEXT, 
...
CREATE TABLE `VILLES` (
  `idville` integer  auto_increment,
  `nom` VARCHAR(255),
  `idpay` smallint(5),
  PRIMARY KEY (`idville`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE `ARTICLES` (
  ...
  `titre` VARCHAR(255),
  `contenu` TEXT,
  `dateparution` datetime DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE `USERS` (
  ...
  `confirmation_token` VARCHAR(150) NULL,
...
CREATE TABLE `PAYS` (
  `idpay` smallint(5) AUTO_INCREMENT,
  `code` int(3),
  `alpha2` varchar(2),
  `alpha3` varchar(3),
  `nom_en_gb` varchar(45),
  `nom_fr_fr` varchar(45),
  PRIMARY KEY (`idpay`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

ALTER TABLE `COMMENTAIRES` ADD FOREIGN KEY (`idusers`) REFERENCES `USERS` (`idusers`);
ALTER TABLE `COMMENTAIRES` ADD FOREIGN KEY (`idarticle`) REFERENCES `ARTICLES` (`idarticle`);
ALTER TABLE `VILLES` ADD FOREIGN KEY (`idpay`) REFERENCES `PAYS` (`idpay`);
ALTER TABLE `ARTICLES` ADD FOREIGN KEY (`idpay`) REFERENCES `PAYS` (`idpay`);

INSERT INTO `PAYS` (`idpay`, `code`, `alpha2`, `alpha3`, `nom_en_gb`, `nom_fr_fr`) VALUES
